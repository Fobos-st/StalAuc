...
from text import current_request, joke_text
import random


async def send_auction_choice(message: types.Message):
    await message.answer(joke_text[random.randint(0, 5)], reply_markup=auction_choice_keyboard)
...
async def send_background_process_choice(message: types.Message):
    ...
    if check_user_request_lot(message.from_user.id):
        await message.answer(text="У вас нету выбранного предмета", reply_markup=background_process_choice_keyboard_0)
    else:
