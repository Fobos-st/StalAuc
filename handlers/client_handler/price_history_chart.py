...
async def get_data_item_more_200(item_id, iteration):
    ...
    data_item = await make_http_get_request(URL_GET_HISTORY_AUC_LOTS.format(item_id), HEADERS_1, PARAMS_CHECKED)
    data_item = json.loads(data_item)
    try:
        data_item = data_item['prices']
        print(data_item, 59)
...
async def parse_json_file(days, item_id):
    data_item = await get_data_item(item_id)
    iteration = 0
    counter = 0
    day = 0

    dates_sold = [str(data_item[counter]["time"])[:10]]
    ...
    while day < days:
        ...
        if await check_time(str(data_item[counter]["time"]), days):
            if dates_sold[day] != str(data_item[counter]["time"])[:10]:
                counter += 1
                day += 1
                dates_sold.append(str(data_item[counter]["time"])[:10])
            ...
            if high_prices[day] < data_item[counter]["price"] / data_item[counter]["amount"]:
                high_prices[day] = data_item[counter]["price"] / data_item[counter]["amount"]
                counter += 1
                value[day] += 1
                continue

            if low_prices[day] > data_item[counter]["price"] / data_item[counter]["amount"]:
                low_prices[day] = data_item[counter]["price"] / data_item[counter]["amount"]
                counter += 1
                value[day] += 1
                continue
...
async def create_table_excel(days, user_id, item_id):
    dates_sold, open_prices, high_prices, low_prices, close_prices, value = await parse_json_file(days, item_id)
    rows = []
...
async def get_count_days(message: types.Message, state: FSMContext):
    ...
    if 0 < days <= 31:
        ...
        await asyncio.sleep(days / 10)
        await msg.delete()
        await message.answer('Рисую график')
        await asyncio.sleep(days / 10)
        filename = await create_table_excel(days, message.from_user.id, data['item_id'])
        print(filename)
        await state.finish()
